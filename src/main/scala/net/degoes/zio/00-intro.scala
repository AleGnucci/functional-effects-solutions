...
object ZIOModel{
  ...
  final case class ZIO[-R, +E, +A](run: R => Either[E, A]) { self =>
    def map[B](f: A => B): ZIO[R, E, B] = ZIO(run.andThen(either => either.map(f)))

    def flatMap[R1 <: R, E1 >: E, B](f: A => ZIO[R1, E1, B]): ZIO[R1, E1, B] = ZIO((env: R1) => {
      run(env) match {
        case Left(error) => Left(error)
        case Right(value) => f(value).run(env)
      }
    })
...
object PrintSequenceZip extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    putStrLn("1").zip(putStrLn("2")).zip(putStrLn("3")) *> ZIO.succeed(ExitCode.success)
}
...
object PrintSequence extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    putStrLn("1") *> putStrLn("2") *> putStrLn("3") *> ZIO.succeed(ExitCode.success)
}
...
object FlatMap extends App {
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    putStrLn("What is your name?") *>
      readLine.flatMap(line => putStrLn(s"Your name is: $line") *> ZIO.succeed(ExitCode.success))
}
...
object PromptName extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    myZipRight(putStrLn("What is your name?"), readLine // using myZipRight instead of *>
   ...
   */
  def myZipRight[R, E, A, B](left: ZIO[R, E, A], right: ZIO[R, E, B]): ZIO[R, E, B] =
...
object ForComprehensionBackward extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    putStrLn("How old are you?").flatMap(_ => readInt)
      .flatMap(age => if (age < 18) putStrLn("You are a kid!") else putStrLn("You are all grown up!"))
...
object NumberGuesser extends App {
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    for {
      ...
      _ <- analyzeAnswer(randomNumber, guessedNumber)
    } yield ExitCode.success
}
